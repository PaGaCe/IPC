...
export default function SwapModal({ swapModal, teams, allPlayersOf, swapPlayer, setSwapModal }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", zIndex: 230, display: "flex", alignItems: "flex-end" }}>
      <div style={{ background: "#15110a", borderTop: "1px solid #2e2615", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 600, margin: "0 auto", padding: "20px 18px 28px", maxHeight: "85vh", overflowY: "auto" }}>
        ...
        <p style={{ color: "#8a7a5a", fontSize: 13, marginBottom: 16 }}>
          <strong style={{ color: "#f0c040" }}>{swapModal.newPlayer.name}</strong> fue fichado pero la plantilla está al límite ({MAX_SQUAD}). Elige a quién liberar:
        </p>
        {allP.filter((p) => p.id !== t?.squad?.star?.id).map((p, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #241e10" }}>
            ...
            <button onClick={() => swapPlayer(swapModal.teamName, p.id, swapModal.newPlayer, swapModal.amount)}
              style={{ background: "#c0392b", border: "none", color: "#fff", borderRadius: 7, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Fichar</button>
          </div>
        ))}
        <button onClick={() => setSwapModal(null)} style={{ ...btn("transparent"), border: "1px solid #2e2615", color: "#8a7a5a", marginTop: 14 }}>Cancelar fichaje</button>
      </div>
    </div>
  );
}
